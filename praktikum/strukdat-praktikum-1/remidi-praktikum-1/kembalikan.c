 ...
 */
 ...
 {
    SinglyList kembalikan;
    slist_init(&kembalikan);
    ...
    for(int i = 0; i < N; i++){
        int temp;
        scanf("%d", &temp);
        slist_pushFront(&kembalikan, temp);
    }
    while (!slist_isEmpty(&kembalikan)) {
        printf("%d ",slist_front(&kembalikan));
        slist_popFront(&kembalikan);
    }
 }
